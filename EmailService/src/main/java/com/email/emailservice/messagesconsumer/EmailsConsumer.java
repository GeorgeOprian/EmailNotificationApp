...
public class EmailsConsumer {
	...
	public EmailsConsumer(EmailService emailService, UserService userService, ObjectMapper mapper, ThreadPoolTaskExecutor taskExecutor) {
	...
	public void receiveEmails(@Payload String fileBody) throws JsonProcessingException, ExecutionException, InterruptedException {
		if (!fileBody.isEmpty()) {
			...
			CompletableFuture.supplyAsync(() -> emailService.sendEmail(request), taskExecutor);
		}
